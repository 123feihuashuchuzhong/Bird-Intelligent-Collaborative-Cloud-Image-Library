package com.tk.yupicturebackend.manager.sharding;
...
public class DynamicShardingManager {
    ...
    private Set<String> fetchAllPictureTableNames() {
        ...
        Set<Long> spaceIds = spaceService.lambdaQuery()
                ...
                .map(space -> space.getSpaceId()) // 使用 id 而不是 getSpaceId()
    ...
    public void createSpacePictureTable(Space space) {
        ...
        if (space.getSpaceType().equals(SpaceTypeEnum.TEAM.getValue()) &&
            ...
            Long spaceId = space.getSpaceId(); // 使用 getId() 而不是 getSpaceId()
            ...
            try {
                ...
                // 更新分表配置
                updateShardingTableNodes();
                log.info("成功创建空间图片分表: {}", tableName);
            } catch (Exception e) {
                log.error("创建图片空间分表失败，空间 id = {}, 错误: {}", space.getSpaceId(), e.getMessage(), e);
            }
        }
    }
